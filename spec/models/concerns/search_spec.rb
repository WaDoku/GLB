...
RSpec.describe Search, type: :concern do
  ...
  describe '#search' do
    ...
    context 'status' do
      it 'returns search result for status' do
        entry.update(status: 'unbearbeitet', japanische_umschrift: 'foo')
        expect(Entry.search('all', 'foo').first).to eq(entry)
        expect(Entry.search('unbearbeitet', 'foo').first).to eq(entry)
        expect(Entry.search('formatiert', 'foo').first).not_to eq(entry)
      end
      it 'returns search case-insensitive result for status' do
        entry.update(status: 'unbearbeitet', japanische_umschrift: 'Foo')
        expect(Entry.search('unbearbeitet', 'foo').first).to eq(entry)
      end
      it 'returns only matches that starts with search pattern' do
        entry.update(status: 'unbearbeitet', japanische_umschrift: 'foo')
        second_entry = create(:entry, status: 'unbearbeitet', japanische_umschrift: 'barfoo')
